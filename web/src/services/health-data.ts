...
export interface HealthDataClient {
  ...
  inspectGoogleFit(): Promise<{ start: string; end: string; data: Record<string, { points: number; latest: number | null; latestAt: string | null; error?: string }> }>
}
...
export const healthDataClient: HealthDataClient = {
  ...
  inspectGoogleFit: () => postJson<{ start: string; end: string; data: Record<string, { points: number; latest: number | null; latestAt: string | null; error?: string }> }>('/.netlify/functions/inspect-google-fit'),
}
